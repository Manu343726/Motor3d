...
Public Structure Foco
    ...
    Public Function RepresentacionFoco(ByRef Camara As DatosCamara) As Point()
        Dim P(3) As Punto3D
        Dim PR(3) As Point
        Return PR
    End Function
...
Public Structure DatosCamara
    ...
    Public ReadOnly Property Posicion() As Punto3D
        Get
            Return CALCTRANS3D_TransformarPunto(New Punto3D(0, 0, 0), MatrizTransformacion)
        End Get
    End Property
...
Public Interface Coordenadas3D
    ...
    Function Representacion2D(ByVal Camara As DatosCamara) As Point
...
Public Structure Punto3D
    ...
    Public Function Representacion2D(ByVal Camara As DatosCamara) As System.Drawing.Point Implements Coordenadas3D.Representacion2D
...
Public Structure Poliedro
    ...
    Public Sub CalcularColores(ByVal Focos() As Foco, ByVal Iluminacion As DatosIluminacion, ByVal Camara As DatosCamara)
    ...
    Public Sub CalcularRepresentaciones(ByVal Camara As DatosCamara)
    ...
    Public Sub RepresentacionCara(ByVal IndiceCara As Long, ByVal Camara As DatosCamara)
...
Public Class ComparadorCaraBuffer
    ...
    Dim mCamara As DatosCamara

    Public Sub New(ByVal Camara As DatosCamara)
        mCamara = Camara
    End Sub
...
#Region "Propiedades"
    ...
    Public ReadOnly Property REPR_Camara() As DatosCamara
        Get
            Return Camara
        End Get
    End Property
...
#Region "Dibujo/Pintado"
    ...
    Public Sub PINT_RedibujarBuffer(ByVal LimpiarSuperficie As Boolean)
        ...
        For i As Integer = 0 To Buffer.Count - 1
            ...
            Select Case mShader
                Case Shader.WireFrameShading
                    CALCSOM_WireFrameShading(MetodoDibujadoPoligonoColor, mFocos, Iluminacion, CaraBuffer, mPoliedros(CaraBuffer.Indices.X).Caras(CaraBuffer.Indices.Y), Camara)
                Case Shader.FlatShading
                    CALCSOM_FlatShading(MetodoRellenoPoligonoColor, mFocos, Iluminacion, CaraBuffer, mPoliedros(CaraBuffer.Indices.X).Caras(CaraBuffer.Indices.Y), Camara, mTeselado)
                Case Shader.GouraudShading
                    CALCSOM_GouraudShading(MetodoRellenoPoligonoColores, mPoliedros(CaraBuffer.Indices.X), CaraBuffer.Indices.Y, Camara, mTeselado)
            End Select
            If mVerIndicesBuffer Then
                g.DrawString(i, New Font("Lucilda Console", 10, FontStyle.Underline, GraphicsUnit.Pixel), Brushes.LightGray, mPoliedros(CaraBuffer.Indices.X).Caras(CaraBuffer.Indices.Y).Baricentro.Coordenadas.Representacion2D(REPR_Camara))
            End If
        Next

        If mVerDatosVertices Then
            For i As Long = 0 To mPoliedros.GetUpperBound(0)
                For j As Long = 0 To mPoliedros(i).Vertices.GetUpperBound(0)
                    g.FillEllipse(Brushes.LightSkyBlue, New Rectangle(mPoliedros(i).Vertices(j).Representacion.X - 2, mPoliedros(i).Vertices(j).Representacion.Y - 1, 4, 4))
                    g.DrawString(j & ": " & mPoliedros(i).Vertices(j).Coordenadas.ToString, New Font("Times New Roman", 10), Brushes.LightSkyBlue, mPoliedros(i).Vertices(j).Coordenadas.Representacion2D(Camara).X, mPoliedros(i).Vertices(j).Coordenadas.Representacion2D(Camara).Y)
                Next
            Next
        End If
        If mVerTriangulacion Then
            Dim Triangulo(2) As Point
            For i As Long = 0 To mPoliedros.GetUpperBound(0)
                For j As Long = 0 To mPoliedros(i).Triangulos.GetUpperBound(0)
                    If CALC3D_OrientacionCara(Camara, mPoliedros(i).Caras(mPoliedros(i).Triangulos(j).IndiceCara)) Then
                        Triangulo(0) = mPoliedros(i).Vertices(mPoliedros(i).Triangulos(j).PrimerVertice).Coordenadas.Representacion2D(Camara)
                        Triangulo(1) = mPoliedros(i).Vertices(mPoliedros(i).Triangulos(j).SegundoVertice).Coordenadas.Representacion2D(Camara)
                        Triangulo(2) = mPoliedros(i).Vertices(mPoliedros(i).Triangulos(j).TercerVertice).Coordenadas.Representacion2D(Camara)

                        g.DrawPolygon(Pens.Yellow, Triangulo)
                    End If
                Next
            Next
        End If
        If mVerNormalesCaras Or mVerNormalesVertices Then
            Dim r As Recta3D
            Dim Vert As Punto3D
            Dim V As Vector3D

            If mVerNormalesCaras Then
                For i As Long = 0 To mPoliedros.GetUpperBound(0)
                    For j As Long = 0 To mPoliedros(i).Caras.GetUpperBound(0)
                        r = New Recta3D(mPoliedros(i).Caras(j).Baricentro.Normal, mPoliedros(i).Caras(j).Baricentro.Coordenadas)

                        g.DrawLine(IIf(mPoliedros(i).Caras(j).CargadaEnBuffer, Pens.LightGreen, Pens.Red), r.PuntoDeLaRecta(0).Representacion2D(Camara), r.PuntoDeLaRecta(-80).Representacion2D(Camara))
                        g.FillEllipse(IIf(mPoliedros(i).Caras(j).CargadaEnBuffer, Brushes.LightGreen, Brushes.Red), New Rectangle(mPoliedros(i).Caras(j).Baricentro.Coordenadas.Representacion2D(Camara).X - 2, mPoliedros(i).Caras(j).Baricentro.Coordenadas.Representacion2D(Camara).Y - 2, 4, 4))
                    Next
                Next
            End If

            If mVerNormalesVertices Then
                For i As Long = 0 To mPoliedros.GetUpperBound(0)
                    For j As Long = 0 To mPoliedros(i).Vertices.GetUpperBound(0)
                        V = mPoliedros(i).Vertices(j).Normal
                        Vert = mPoliedros(i).Vertices(j).Coordenadas
                        r = New Recta3D(V, Vert)

                        g.DrawLine(Pens.Purple, r.PuntoDeLaRecta(0).Representacion2D(Camara), r.PuntoDeLaRecta(-40).Representacion2D(Camara))
                        g.FillEllipse(Brushes.Purple, New Rectangle(Vert.Representacion2D(Camara).X - 2, Vert.Representacion2D(Camara).Y - 2, 4, 4))
                    Next
                Next
            End If
        End If

        If SeñalarCaraBuffer Then
            If CaraSeñalada > Buffer.Count - 1 Then CaraSeñalada = Buffer.Count - 1
            g.DrawPolygon(Pens.LightGreen, mPoliedros(Buffer(CaraSeñalada).Indices.X).Caras(Buffer(CaraSeñalada).Indices.Y).Representacion)
        End If



        If DibujarEjes Then
            g.DrawLine(Pens.Red, New Punto3D(0, 0, 0).Representacion2D(Camara), New Punto3D(10000, 0, 0).Representacion2D(Camara))
            g.DrawLine(Pens.Green, New Punto3D(0, 0, 0).Representacion2D(Camara), New Punto3D(0, 10000, 0).Representacion2D(Camara))
            g.DrawLine(Pens.Blue, New Punto3D(0, 0, 0).Representacion2D(Camara), New Punto3D(0, 0, 10000).Representacion2D(Camara))
        End If

        If PintarFocos Then
            Dim Punto As Point
            Dim R As Recta3D
            Dim Colores(0) As Color
            For Each Foco As Foco In mFocos
                Punto = Foco.Coordenadas.Representacion2D(Camara)
                If Foco.Tipo <> TipoFoco.Puntual Then
                    R = New Recta3D(Foco.VectorDireccion.VectorUnitario, Foco.Coordenadas)
                    g.DrawLine(New Pen(Foco.Color, 2), R.PuntoDeLaRecta(0).Representacion2D(Camara), R.PuntoDeLaRecta(100).Representacion2D(Camara))
                    g.FillEllipse(New SolidBrush(Foco.Color), New Rectangle(Foco.Coordenadas.Representacion2D(Camara).X - 4, Foco.Coordenadas.Representacion2D(Camara).Y - 4, 8, 8))
                Else
                    g.FillEllipse(New SolidBrush(Foco.Color), New Rectangle(Foco.Coordenadas.Representacion2D(Camara).X - 4, Foco.Coordenadas.Representacion2D(Camara).Y - 4, 8, 8))
                End If
            Next
    ...
    Public Sub PINT_Repintar()
        If PINT_Sombreado Then PINT_RedibujarSombreado(True)
        PINT_RedibujarBuffer(Not PINT_Sombreado)
        Superficie.BackgroundImage = BMPDibujo
        Superficie.Refresh()
    End Sub
...
#Region "Buffer de dibujo"
    ...
    Public Sub BUFF_ReasignarColoresBuffer()
        Dim t As DateTime = Now
        TiempoReasignacion = (Now - t).TotalMilliseconds
        RaiseEvent BUFF_FinReasignacionBuffer()
